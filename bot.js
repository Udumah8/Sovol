...
class VolumeBoosterBot {
  constructor() {
    ...
    this.masterPrivateKey = process.env.MASTER_PRIVATE_KEY ? JSON.parse(process.env.MASTER_PRIVATE_KEY) : null;
    this.masterKeypair = this.masterPrivateKey ? Keypair.fromSecretKey(new Uint8Array(this.masterPrivateKey)) : null;
    this.withdrawToPrivateKey = process.env.WITHDRAW_TO_PRIVATE_KEY ? JSON.parse(process.env.WITHDRAW_TO_PRIVATE_KEY) : null;
    this.withdrawToKeypair = this.withdrawToPrivateKey ? Keypair.fromSecretKey(new Uint8Array(this.withdrawToPrivateKey)) : null;
    ...
    this.initialMasterBalance = 0;

    if (this.masterKeypair && !this.isDevnet) console.log(`Master: ${this.masterKeypair.publicKey.toBase58()}`);
    if (this.withdrawToKeypair) console.log(`Sink: ${this.withdrawToKeypair.publicKey.toBase58()}`);
  ...
  async fundSingleWallet(wallet) {
    ...
    return this.withRetry(async () => {
      ...
      if (!this.masterKeypair) throw new Error('No master for mainnet');
      const tx = new Transaction().add(SystemProgram.transfer({
        fromPubkey: this.masterKeypair.publicKey,
        toPubkey: pubkey,
  ...
  async checkCircuitBreakers() {
    ...
    if (this.masterKeypair && this.initialMasterBalance > 0) {
      const currentBalance = await this.connection.getBalance(this.masterKeypair.publicKey);
      const loss = ((this.initialMasterBalance - currentBalance) / this.initialMasterBalance) * 100;
  ...
  // Smart Rebalancing Methods
  async rebalanceWallets() {
    if (!this.enableRebalancing || !this.masterKeypair) {
      console.log('⚖️  Rebalancing skipped (disabled or no master wallet)');
      return;
    }

    console.log('\n⚖️  === Smart Rebalancing Started ===');

    // Only rebalance active wallets to avoid memory issues with 100k
    ...
    // Identify wallets needing funds
    const needsFunding = walletBalances.filter(wb => wb.balance < this.minWalletBalance);
    const hasSurplus = walletBalances.filter(wb => wb.balance > this.targetWalletBalance * 1.5);

    console.log(`Wallets needing funds: ${needsFunding.length}`);
    console.log(`Wallets with surplus: ${hasSurplus.length}`);

    // Consolidate dust from surplus wallets
    if (hasSurplus.length > 0) {
      await this.consolidateDust(hasSurplus);
    }

    // Fund low-balance wallets from master
    if (needsFunding.length > 0) {
      await this.fundLowBalanceWallets(needsFunding);
    }

    console.log('⚖️  === Rebalancing Complete ===\n');
  }

  async consolidateDust(surplusWallets) {
    console.log('Consolidating surplus funds to master...');
    ...
    for (const { wallet, balance } of surplusWallets) {
      ...
      if (surplus > this.dustThreshold) {
        try {
          const transferAmount = surplus - (0.0001 * LAMPORTS_PER_SOL); // Leave fee buffer
          const tx = new Transaction().add(SystemProgram.transfer({
          ...
          console.log(`  ✓ ${wallet.name}: Consolidated ${(transferAmount / LAMPORTS_PER_SOL).toFixed(4)} SOL`);
        } catch (e) {
          console.error(`  ✗ ${wallet.name}: Consolidation failed - ${e.message}`);
        }
      }
    }
  }

  async fundLowBalanceWallets(lowBalanceWallets) {
    console.log('Funding low-balance wallets from master...');

    const masterBalance = await this.connection.getBalance(this.masterKeypair.publicKey);
    const totalNeeded = lowBalanceWallets.reduce((sum, wb) =>
      sum + (this.targetWalletBalance - wb.balance), 0
    );

    if (masterBalance < totalNeeded + (0.01 * LAMPORTS_PER_SOL)) {
      console.warn('⚠️  Insufficient master balance for full rebalancing');
      return;
    }

    for (const { wallet, balance } of lowBalanceWallets) {
      const needed = this.targetWalletBalance - balance;
      ...
      try {
        ...
        const sig = await this.connection.sendTransaction(tx, [this.masterKeypair], {
          skipPreflight: true,
          maxRetries: 2
        });
        await this.connection.confirmTransaction(sig, 'confirmed');

        console.log(`  ✓ ${wallet.name}: Funded ${(needed / LAMPORTS_PER_SOL).toFixed(4)} SOL`);
      } catch (e) {
        console.error(`  ✗ ${wallet.name}: Funding failed - ${e.message}`);
      }
    }
  }
  ...
  async withdrawAllFunds() {
    if (!this.withdrawToKeypair) {
      console.log('No sink. Skipping withdrawal.');
      return;
    }
    ...
    const sinkBalance = await this.connection.getBalance(this.withdrawToKeypair.publicKey);
    console.log(`Final sink SOL: ${(sinkBalance / LAMPORTS_PER_SOL).toFixed(4)}`);

    // Sink Token Cleanup
    if (this.withdrawToKeypair) {
      const sinkTokenAcc = getAssociatedTokenAddressSync(this.memeMint, this.withdrawToKeypair.publicKey);
      try {
        const info = await this.connection.getAccountInfo(sinkTokenAcc);
        if (info && parseInt(info.data.slice(64, 72), 'hex') === 0) { // balance == 0
          const closeTx = new Transaction().add(
            createCloseAccountInstruction(sinkTokenAcc, this.withdrawToKeypair.publicKey, this.withdrawToKeypair.publicKey)
          );
          await this.connection.sendTransaction(closeTx, [this.withdrawToKeypair]);
  ...
  async withdrawSingleWallet(wallet) {
    return this.withRetry(async () => {
      ...
      if (solBalance > this.minWithdrawSol) {
        ...
        const tx = new Transaction().add(SystemProgram.transfer({
          fromPubkey: wallet.keypair.publicKey,
          toPubkey: this.withdrawToKeypair.publicKey,
          lamports: transferAmount,
        }));
  ...
  async init() {
    ...
    if (this.masterKeypair) {
      this.initialMasterBalance = await this.connection.getBalance(this.masterKeypair.publicKey);
      console.log(`Initial master balance locked: ${(this.initialMasterBalance / LAMPORTS_PER_SOL).toFixed(4)} SOL`);
    }
  ...
  async stop() {
    ...
    if (this.withdrawToKeypair) await this.withdrawAllFunds();
